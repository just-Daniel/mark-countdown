...
function playTick() {
    ...
    gain.gain.exponentialRampToValueAtTime(0.001, tickAudioCtx.currentTime + 0.08);
...
function initHearts() {
    ...
    function drawHeart(x, y, size, opacity) {
        ...
        ctx.bezierCurveTo(x - size / 2, topY + size / 2, x, topY + size * 0.65, x, topY + size);
        ctx.bezierCurveTo(x, topY + size * 0.65, x + size / 2, topY + size / 2, x + size / 2, topY + size / 4);
...
function launchConfetti() {
    ...
    const particles = [];
    const colors = ["#f857a6", "#ff5858", "#00b894", "#ffeaa7", "#74b9ff", "#a29bfe", "#fd79a8"];
...
function initShare() {
    $("share-btn").addEventListener("click", async () => {
        ...
        if (navigator.share) {
            try {
                await navigator.share({ title: "Стосунки Марка", text });
            } catch {}
        } else {
